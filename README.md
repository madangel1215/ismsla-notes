...
- **九大視圖**：五張圖（PDCA / 風險 / AI 生命週期 / AIMS 42001 / 稽核流程 19011）＋「附錄A」「文件·標準」清單＋「關係總覽」知識圖（fcose 力導向，只顯示有關係的節點、大小＝連線數、可篩選）＋「考題練習」。
- **考題練習**：純練習題庫（`data/exam.json`），三模式 — **練習**（作答即時對答＋解析）／**模擬考**（隨機抽題、計時、交卷出分＋逐題檢討）／**錯題本**（自動收答錯的題重練）；每題解析下方「相關」可一鍵跳回對應條款；作答進度存本機（localStorage）。
...
| `1` – `9` | 切換九個視圖（PDCA／風險／AI／AIMS／稽核流程／附錄A／文件·標準／關係總覽／考題練習） |
| `g` | 切換 學習者 ↔ 稽核者(LA) 視角 |
| `t` | 切換 深／淺色 |
| `\` | 收合／展開左側選單 |
| `/` | 聚焦搜尋 |
